...
(
    ...
    ID
    ,NAME
    FROM PPRO_AIRFLOW_DB.PPRO_AIRFLOW_SCHEMA.CREDITS_DIM
    WHERE ROLE='ACTOR'
),
MOVIE_DETAILS as (select * from PPRO_AIRFLOW_DB.PPRO_AIRFLOW_SCHEMA.SHOW_DETAILS_DIM)

SELECT
REPLACE(REPLACE(UPPER(GENRES),'[',''),']','') AS GENRES
,ACTORS.NAME
,SUM(CASE WHEN GENRES IS NOT NULL THEN 1 ELSE 0 END) AS PERFORMANCES 
FROM MOVIE_DETAILS as SD
LEFT JOIN
ACTORS
ON SD.ID=ACTORS.ID
